...
public class PessoaService {
    ...
    public Pessoa cadastrarPessoa(PessoaCreateDTO pessoa) {
        try {
            ...
            e.printStackTrace();  // Isso vai logar a stack trace completa no console
            throw new ApiException(HttpStatus.BAD_REQUEST, new ErrorResponse(HttpStatus.BAD_REQUEST.toString(), e.getMessage()));
        }
    }

    public List<Pessoa> cadastrarPessoas(List<PessoaCreateDTO> pessoas) {
        List<Pessoa> novasPessoas = new ArrayList<>();

        for (PessoaCreateDTO dto : pessoas) {
            Pessoa pessoaEntity = new Pessoa();
            pessoaEntity.setNome(dto.getNome());
            pessoaEntity.setNomeFamilia(dto.getNomeFamilia());
            pessoaEntity.setEndereco(dto.getEndereco());
            pessoaEntity.setNumero(dto.getNumero());
            pessoaEntity.setComplemento(dto.getComplemento());
            pessoaEntity.setBairro(dto.getBairro());
            pessoaEntity.setCep(dto.getCep());
            pessoaEntity.setMunicipio(dto.getMunicipio());
            pessoaEntity.setUf(dto.getUf());
            pessoaEntity.setTelefone1(dto.getTelefone1());
            pessoaEntity.setTelefone2(dto.getTelefone2());
            pessoaEntity.setNascimento(dto.getNascimento());
            pessoaEntity.setBatismo(dto.getBatismo());
            pessoaEntity.setEmail(dto.getEmail());
            pessoaEntity.setGenero(dto.getGenero());
            pessoaEntity.setAnciao(dto.getAnciao());
            pessoaEntity.setServoMinisterial(dto.getServoMinisterial());
            pessoaEntity.setPioneiroRegular(dto.getPioneiroRegular());
            pessoaEntity.setUngido(dto.getUngido());
            pessoaEntity.setContatoNome(dto.getContatoNome());
            pessoaEntity.setContatoParentesco(dto.getContatoParentesco());
            pessoaEntity.setContatoTelefone(dto.getContatoTelefone());
            pessoaEntity.setContatoTelefone1(dto.getContatoTelefone1());
            pessoaEntity.setContatoEmail(dto.getContatoEmail());

            novasPessoas.add(pessoaRepository.save(pessoaEntity));
        }
